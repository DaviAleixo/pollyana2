...
export default function ProductFormNew() {
  ...
  // Efeito para carregar todas as categorias e inicializar os dados do formulário
  useEffect(() => {
    const fetchedAllCategories = categoriesService.getAll();
    setAllCategories(fetchedAllCategories);
    setMainCategories(fetchedAllCategories.filter(c => c.parentId === null));

    if (isEditing && id) {
      const product = productsService.getById(parseInt(id));
      if (product) {
        setFormData({
          nome: product.nome,
          preco: product.preco,
      ...
      setSelectedMainCategoryId(1); // Também define a categoria principal como 'Todos'
    }
